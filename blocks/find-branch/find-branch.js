...
const SAMPLE_DATA = [
  { name: 'Lloyds Bank — High Street', address: '24 High Street, London, EC1A 2BN', phone: '0345 602 1997', hours: 'Mon–Fri 9:00–17:00, Sat 9:30–12:30' },
  { name: 'Lloyds Bank — Cheapside', address: '101 Cheapside, London, EC2V 6DT', phone: '0345 300 0000', hours: 'Mon–Fri 9:30–16:30, Sat closed' },
];
...
function render(block, branches, bridge) {
  if (branches && branches.length > 0) {
    renderResults(block, branches, bridge);
  } else {
...
function renderEmpty(block, bridge) {
  ...
  card.style.cssText = `background:${theme?.bg ?? '#0a4a31'};color:${theme?.fg ?? '#fff'}`;

  const icon = document.createElement('div');
  icon.className = 'find-branch-pin';
  icon.appendChild(pinIcon(32, theme?.fg ?? '#fff'));
  ...
  input.setAttribute('aria-label', 'Postcode or town');
  ...
  const submit = () => {
    ...
    if (bridge && q) bridge.sendMessage(`Find Lloyds Bank branches near ${q}`);
  };
...
function renderResults(block, branches, bridge) {
  const row = document.createElement('div');
  row.className = 'find-branch-row';
